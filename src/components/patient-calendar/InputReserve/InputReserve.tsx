...
const InputReserve = ({ dateArg }: Props) => {
    ...
    const reservedDateYMDHM: string = `${reservedDateYear}年${reservedDateMonth}月${reservedDateDay}日 ${reservedDateHour}時 ${reservedDateMinute}`
    ...
    const handleConfirm = () => {
        ...
        // newAlert.length == 0 ? alert('確認画面移行可能です') : setAlertMessage(newAlert)
        if (newAlert.length == 0) {
            alert('確認画面移行可能です')
        } else {
            // newAlert.unshift('次の内容を確認してください:')
            setAlertMessage(newAlert)
        }
        //! setIsOpenConfirm(true) あとでalert('確認画面移行可能です')と差し替える
    }
    return (
        <Dialog
            open={isOpenForm}
